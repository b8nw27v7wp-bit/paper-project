...
- **系统即智能体**：`app/agents/system_agent.py:14` `SystemAgent` 封装 `POST /plans` + `SSE` + `CLI studying` 为单一对外 `Agent`，`name: studying-planner`，`tools: registry.list_tools_detailed()`，`trace_id` 即 `session_id`，外部 `hermes/pi` 可 `mcp:call studying:plan`。
...
apps/server/{api(16组66端点 app/api/v1/* + api/workbench),agents(6节点 graph.py:351 + checkpoint + system_agent.py:14 SystemAgent 单点),services(memory asearch),rag,graph,mcp,scheduler}
...
| 2026-09-01 | v0.7 | 同步 SystemAgent: app/agents/system_agent.py:14（2处，C4 已含 SystemAgent 跳过）、管理系统→管理的智能体 |
